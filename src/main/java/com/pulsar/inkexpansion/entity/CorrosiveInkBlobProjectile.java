...
public class CorrosiveInkBlobProjectile extends ProjectileEntity {
    public CorrosiveInkBlobProjectile(EntityType<? extends ProjectileEntity> entityType, World world) {
    ...
    public void tick() {
        ...
        this.setYaw(this.getYaw() + 11.26f);
        this.setPitch(this.getPitch() + 9.87f);
            ...
            for (int y = -MathHelper.ceil(radius); y <= MathHelper.ceil(radius); y++) {
                for (int z = -MathHelper.ceil(radius); z <= MathHelper.ceil(radius); z++) {
                    ...
                    if (center.toCenterPos().distanceTo(target.toCenterPos()) < radius) {
                        ...
                        if ((state.isReplaceable() || state.isOf(InkExpansion.CORROSIVE_INK)) && state.getFluidState().isEmpty()) {
                            BlockState newState = state.isOf(InkExpansion.CORROSIVE_INK) ? state : InkExpansion.CORROSIVE_INK.getDefaultState();
    ...
    private void damageNearby() {
        for (LivingEntity living : this.getWorld().getEntitiesByClass(LivingEntity.class, Box.of(this.getPos(),
            ...
            float damage = 12f * (4f - (float)this.getPos().distanceTo(living.getPos())) / 4f;
            living.damage(this.getDamageSources().create(InkExpansion.INK_DAMAGE_TYPE), damage);
            living.addStatusEffect(new StatusEffectInstance(InkExpansion.INKED, 180, 0));
            living.addStatusEffect(new StatusEffectInstance(InkExpansion.CORRODING, 120, 0));
        }
    }
}
