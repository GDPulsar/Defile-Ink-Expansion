...
public class CorrosiveInkThrowerItem extends Item {
    private final int maxFuel = 10000;

    public CorrosiveInkThrowerItem() {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (stack.getOrCreateNbt().contains("fuel")) {
            ...
            if (fuel > 0) {
                stack.getOrCreateNbt().putInt("fuel", fuel - 250);
                CorrosiveInkBlobProjectile ink = new CorrosiveInkBlobProjectile((PlayerEntity)user, world);
