...
public class CorrosiveInkSpreaderItem extends Item {
    private final int maxFuel = 10000;

    public CorrosiveInkSpreaderItem() {
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        if (stack.getOrCreateNbt().contains("fuel") && remainingUseTicks % 2 == 0) {
            ...
            if (fuel > 0) {
                stack.getOrCreateNbt().putInt("fuel", fuel - 50);
                for (int i = -1; i <= 1; i++) {
                    CorrosiveInkProjectile ink = new CorrosiveInkProjectile((PlayerEntity)user, world);
